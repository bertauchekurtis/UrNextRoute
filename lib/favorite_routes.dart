          ...
          if (appState.favoritePaths.isNotEmpty) ...[
            for (var path in appState.favoritePaths)
              ListTile(
                leading: GestureDetector(
                  ...
                  onTap: () {
                    showDialog(context: context, builder: (context) {
                      return AlertDialog(
                        title: const Text("Are you sure you want to remove this path?"),
                        content: const Text("This action cannot be undone."),
                        actions: [
                          ElevatedButton(
                            onPressed: () => {
                              appState.favoritePaths.remove(path),
                              if(appState.pathObj == path) {
                ...
                title: Text("From ${appState.getClosestBuilding(path.getPathList()[0])} to ${appState.getClosestBuilding(path.getPathList()[path.getPathList().length - 1])}"),
                subtitle: Text(
                    "Tap to Open on Map"),
                ...
                onTap: () => {
                  ...
                  appState.end = StartEnd(false, path.getPathList()[path.getPathList().length - 1]),
                  appState.triggerUpdate(),
                },
              ),
          ]else ...[
